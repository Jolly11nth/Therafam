...
type View = 'welcome' | 'anonymous' | 'auth' | 'dashboard' | 'ai' | 'programs' | 'messages' | 'settings' | 'therapist';
...
export default function TherafamApp() {
  ...
  const [session, setSession] = useState<Session | null>(() => {
    const raw = localStorage.getItem('therafam:session');
    return raw ? JSON.parse(raw) : null;
  });

  useEffect(() => {
    if (session) localStorage.setItem('therafam:session', JSON.stringify(session));
  }, [session]);

  function openAuth(mode: AuthMode) { setAuthMode(mode); setView('auth'); }
  function signOut() { localStorage.removeItem('therafam:session'); localStorage.removeItem('therafam:userId'); setSession(null); setView('welcome'); }
  ...
  return (
    <section className={`landing-shell ${ready ? 'intro-ready' : ''}`}>
      <div className="landing-content">
        <div className="landing-anim landing-heading"><Brand /></div>
        <h1 className="landing-anim landing-heading">Your Safe Space for Mental<br className="desktop-only" /> Wellness.</h1>
        ...
        <select className="language-select landing-anim" defaultValue="English" aria-label="Language"><option>English</option><option>Español</option></select>
...
function AnonymousMode({ onBack, onSignIn, onSignUp }: { onBack: () => void; onSignIn: () => void; onSignUp: () => void }) {
  return (
    <section className="anonymous-shell">
      <header className="anonymous-topbar"><button className="workspace-brand" onClick={onBack}><Brand compact /></button><button className="theme-button">☼ Light</button></header>
      <div className="anonymous-content">
        ...
        <div className="anonymous-heading"><h1>Welcome to Therafam</h1><p>Explore our mental wellness features anonymously</p></div>
        <article className="anonymous-feature-card"><span>Failed to fetch</span><button className="primary-action" onClick={() => window.location.reload()}>Try Again</button></article>
        <article className="privacy-card"><span className="privacy-icon">♧</span><strong>Your Privacy Matters</strong><p>In anonymous mode, your data is stored only in your browser and will be cleared when you close the app. For a persistent experience with data synced across devices, create a free account.</p></article>
...
function Auth({ mode, setMode, onBack, onGuest, onTherapist, onSuccess }: { mode: AuthMode; setMode: (mode: AuthMode) => void; onBack: () => void; onGuest: () => void; onTherapist: () => void; onSuccess: (session: Session) => void }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      if (apiBase) {
        const response = await callBackend(`/api/${mode === 'signin' ? 'auth/signin' : 'auth/signup'}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(mode === 'signin' ? { email, password } : { email, password, userType: 'client', fullName: name }) });
        const data = await response.json();
        if (!data.success) throw new Error(data.error ?? 'Authentication failed');
        onSuccess({ userId: data.userId ?? data.user?.id ?? crypto.randomUUID(), email, userType: 'client' });
      } else {
        onSuccess({ userId: crypto.randomUUID(), email: email || 'demo@therafam.local', userType: 'client' });
      }
  ...
  return (
    <section className="auth-shell"><div className="auth-card"><button className="mobile-back" onClick={onBack}>‹</button><Brand compact /><div className="auth-tabs"><button className={mode === 'signin' ? 'tab active' : 'tab'} onClick={() => setMode('signin')}>Sign In</button><button className={mode === 'signup' ? 'tab active' : 'tab'} onClick={() => setMode('signup')}>Sign Up</button></div>
      <div className="demo-card"><div className="demo-icon">🔑</div><div><strong>{mode === 'signin' ? 'Demo Account Available' : 'Create your Therafam account'}</strong><p>{mode === 'signin' ? 'Connect the backend to use your real account. Demo mode keeps the UI testable locally.' : 'Your account will unlock saved mood history, programs, settings, and secure care features.'}</p></div></div>
