...
import { copy, getLanguage, getTheme, setLanguage, applyTheme, type Language, type Theme } from './lib/preferences';
...
function usePreferences() {
  ...
  useEffect(() => {
    const sync = () => { setLanguageState(getLanguage()); setThemeState(getTheme()); };
    window.addEventListener('therafam:preferences', sync);
    return () => window.removeEventListener('therafam:preferences', sync);
  }, []);
  return { language, theme, setLanguage: (value: Language) => { setLanguage(value); setLanguageState(value); }, setTheme: (value: Theme) => { applyTheme(value); setThemeState(value); } };
}

function ThemeButton() {
  const { theme, setTheme } = usePreferences();
  const next: Theme = theme === 'dark' ? 'light' : 'dark';
  return <button className="theme-button" aria-label={`Switch to ${next} mode`} title={`Switch to ${next} mode`} onClick={() => setTheme(next)}>{theme === 'dark' ? '☀' : '☾'} {theme === 'dark' ? 'Dark' : 'Light'}</button>;
}

function Welcome({ onSignIn, onSignUp, onGuest }: { onSignIn: () => void; onSignUp: () => void; onGuest: () => void }) {
  const [ready, setReady] = useState(false);
  const { language, setLanguage: changeLanguage } = usePreferences();
  ...
  return <section className={`landing-shell ${ready ? 'intro-ready' : ''}`}><div className="landing-content">
    ...
    <div className="landing-logo-card landing-anim"><Brand /></div>
    <div className="landing-preferences"><select className="language-select landing-anim" value={language} aria-label={t.language} onChange={(e) => changeLanguage(e.target.value as Language)}><option value="en">English</option><option value="es">Español</option></select><ThemeButton /></div>
    <div className="landing-actions"><button className="outline-action landing-anim" onClick={onSignIn}>{t.signIn}</button><button className="primary-action landing-anim" onClick={onSignUp}>{t.signUp}</button></div>
    <div className="landing-lower landing-anim"><button className="link-button guest" onClick={onGuest}>{t.guest}</button><p className="anonymous-copy">Explore the app anonymously. Data won’t be saved permanently.</p><p className="privacy-copy">Your privacy is important to us, all conversations are end-to-end<br className="desktop-only" /> encrypted.</p></div>
  </div></section>;
}
...
function AnonymousMode({ onBack, onSignIn, onSignUp }: { onBack: () => void; onSignIn: () => void; onSignUp: () => void }) {
  ...
  const t = copy[language];
  return <section className="anonymous-shell"><header className="anonymous-topbar"><button className="workspace-brand" onClick={onBack}><Brand compact /></button><ThemeButton /></header><div className="anonymous-content">
    ...
    <article className="privacy-card"><span className="privacy-icon">♧</span><strong>Your Privacy Matters</strong><p>In anonymous mode, your data stays in your browser and is not linked to an account. For a persistent experience with data synced across devices, create a free account.</p></article><button className="text-link" onClick={onBack}>{t.backHome}</button>
  </div></section>;
}
...
function Auth({ mode, setMode, onBack, onGuest, onTherapist, onSuccess }: { mode: AuthMode; setMode: (mode: AuthMode) => void; onBack: () => void; onGuest: () => void; onTherapist: () => void; onSuccess: (session: Session) => void }) {
  ...
  const { language } = usePreferences(); const t = copy[language];
