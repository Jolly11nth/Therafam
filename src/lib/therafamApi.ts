...
export async function sendAiMessage(message: string, history: ChatMessage[] = [], userId?: string) {
  if (!apiBase) {
    return {
      response:
        'I’m here with you. The AI service is currently in demo mode, but your message was received. Connect the backend to enable live responses.',
      demo: true,
    };
  }
...
export async function getPrograms() {
  ...
  const { data, error } = await supabase
    ...
    .select('id,title,description,category,difficulty_level,estimated_duration_days,thumbnail_url,tags,total_lessons')
...
export async function getLessons(programId: string) {
  ...
  const { data, error } = await supabase
    .from('lessons')
    .select('id,program_id,title,description,lesson_number,content_type,content_text,duration_minutes,learning_objectives,key_concepts,exercises')
